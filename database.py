...
class d12:
  ...
  class config:
    @staticmethod
    def init(config):
      """example config : {'pass_through' : bool, 'req_hit_cache' : bool}"""
      db.core.cdict(config, d12.vars.X_config)
  ...
  class cache:
    ...
    def read(k):
      try:
        ...
        if d12.vars.X_config['pass_through'] == True and db.off_site.check() == "IsOffSite":
          return db.off_site.index(k)
        elif d12.vars.X_config['pass_through'] == True and db.on_site.check() == "IsLocal":
          return db.on_site.index(k)
        if d12.vars.X_config['pass_through'] == False:
